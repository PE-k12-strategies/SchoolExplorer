-- Fix nces_map_school_points: do NOT aggregate enrollment for the whole US
-- when only one state is requested (that caused statement timeouts / HTTP 500).
-- Safe to re-run.
...
begin
  ...
  -- Require a scope — nationwide school points are too large for one RPC call.
  ...
  with schools as (
    select
      ...
      sd.school_level,
      sd.charter,
      sd.lowest_grade_offered,
      sd.highest_grade_offered,
      sd.teachers_fte
  ...
  enroll as (
    ...
    where se.school_year = yr
      ...
      and exists (select 1 from schools s where s.ncessch = se.ncessch)
    group by se.ncessch
  )
  select
    s.ncessch,
    ...
    s.school_level,
    s.charter,
    s.lowest_grade_offered as lowest_grade,
    s.highest_grade_offered as highest_grade,
    coalesce(e.enrollment, 0)::bigint,
    s.teachers_fte
  from schools s
  left join enroll e on e.ncessch = s.ncessch
  order by s.ncessch;
end;
$$;
